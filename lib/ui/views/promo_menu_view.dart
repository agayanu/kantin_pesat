...
class PromoMenuView extends StatefulWidget {
  const PromoMenuView({Key? key}) : super(key: key);

  @override
  _PromoMenuViewState createState() => _PromoMenuViewState();
}

class _PromoMenuViewState extends State<PromoMenuView> {
  ...
  void _loadMenu() async {
    ...
    try {
      ...
      final menuUrl = '$baseURL/menu-diskon?page=$_page';
      if (kDebugMode) {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        ...
        final menuUrl = '$baseURL/menu-diskon?page=$_page';
        if (kDebugMode) {
          print('[MenuDiskonLoad URL] $menuUrl');
        }
        final response = await client.get(Uri.parse(menuUrl), headers: {
          'Authorization': 'Bearer $token',
        });
